...
// time based off computer not fps// Get game timer
const startTime = Date.now();
...
class Player {
    constructor(x, y) {
        ...
        this._maxHealth = 100; // Maximum health (use backing field to avoid accessor recursion)
    ...
    // Getters for player aspects
    getPosition() { return Object.freeze({ xGU: this.xGU, yGU: this.yGU }); }
    getSize() { return Object.freeze({ widthGU: this.widthGU, heightGU: this.heightGU }); }
    getColor() { return this.color; }
    getHealth() { return this.health; }
    getMaxHealth() { return this.maxHealth; }

    setPosition({ xGU, yGU }) { 
        this.xGU = (xGU === undefined) ? this.xGU : xGU;
        this.yGU = (yGU === undefined) ? this.yGU : yGU;
    }
    setSize({ widthGU, heightGU }) {
        this.widthGU = (widthGU === undefined) ? this.widthGU : widthGU;
        this.heightGU = (heightGU === undefined) ? this.heightGU : heightGU;
    }
    setColor(color) { this.color = color; }
    setHealth(health) { this.health = health; }
    setMaxHealth(maxHealth) { this._maxHealth = maxHealth; }

    // Movement getters and setters
    getSpeed() { return Math.sqrt(this.dxGU * this.dxGU + this.dyGU * this.dyGU); } // returns current speed
    getVelocity() { return { dxGU: this.dxGU, dyGU: this.dyGU }; } // returns horizontal and vertical speed
    getAcceleration() { return this.accelerationGU; } // returns acceleration
    getMaxSpeed() { return this.maxSpeedGU; } // returns max speed
    getFrictionValue() { return this.friction; } // returns friction
    
    setVelocity({ dxGU, dyGU }) {
        this.dxGU = (dxGU === undefined) ? this.dxGU : dxGU;
        this.dyGU = (dyGU === undefined) ? this.dyGU : dyGU;
    }
    setAcceleration(accelerationGU) { this.accelerationGU = accelerationGU; }
    setMaxSpeed(maxSpeedGU) { this.maxSpeedGU = maxSpeedGU; }
    setFrictionValue(friction) { this.friction = friction; }

    sprintRecharge(deltaTime) {
